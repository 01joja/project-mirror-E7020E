...
pub mod hid {
    ...
    */
    ...
    // From https://www.microchip.com/forums/m391435.aspx
    // Where they make references to windows vista and that might work.
    /*
    const REPORT_DESCR: &[u8] = &[
        0x05, 0x01,        // USAGE_PAGE (Generic Desktop)
        0x09, 0x02,        // USAGE (Mouse)
...
const APP: () = {
    ...
    fn on_tick(mut cx: on_tick::Context) {
        ...
        *counter = (*counter + 1) % P;
        ...
        //Writes the report to the pc.
        if *counter == 4 {
            led.set_high().ok();
            hid.write(&hid::report(16, 0, 0));
        } else if *counter == 3 {
            led.set_low().ok();
            hid.write(&hid::report(8, 0, 0));
        } else if *counter == 2 {
            led.set_low().ok();
            hid.write(&hid::report(4, 0, 0));
        } else if *counter == 1 {
            led.set_low().ok();
            hid.write(&hid::report(2, 0, 0));
        } else {
            led.set_low().ok();
            hid.write(&hid::report(1, 0, 0));
        }
    }
